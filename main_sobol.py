...
def quality_diversity(function, instance, dimension, initial_size, size_best, lb, ub, iterations):
    ...
    # Converte lower_bound e upper_bound in array NumPy
    lower_bound = np.array([lb] * dimension)
    upper_bound = np.array([ub] * dimension)
    ...
    # Genera il campione Sobol
    sobol_sample = i4_sobol_generate(dimension, initial_size)

    # Scala il campione Sobol nei limiti specificati
    sobol_sample = lower_bound + sobol_sample * (upper_bound - lower_bound)

    # Usa il campione Sobol
    random_sample = sobol_sample
