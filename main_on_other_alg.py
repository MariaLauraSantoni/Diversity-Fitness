...
def quality_diversity(function, instance, dimension, initial_size, size_best, lb, ub, iterations):
    ...
    # Inizializza un array vuoto per contenere le coppie (x1, x2)
    vettori = []
    # Apre il file in lettura
    with open(str(inp1) + '_output.txt', 'r') as file:
        # # Salta la prima riga (etichette delle colonne)
        # next(file)
        # Legge le righe rimanenti
        for line in file:
            # Splitta la riga in colonne
            colonne = line.strip().split()

            # Estrai i valori e li converte in float se necessario
            x1 = float(colonne[1])
            x2 = float(colonne[2])
            #x3 = float(colonne[3])
            #x4 = float(colonne[4])
            ...
            #x8 = float(colonne[8])
            #x9 = float(colonne[9])
            #x10 = float(colonne[10])
            
            # Aggiungi la coppia (x1, x2) alla lista
            vettore = [x1, x2,]
            vettori.append(vettore)
    # Prendi solo i primi 10000 punti
    vettori_primi_10000 = vettori[:initial_size]
    # Converti la lista in un array numpy
    random_sample = np.array(vettori_primi_10000)
    ...
    with open(str(inp1)+ '_my_best_batch.txt', 'w') as output_file:
        # Scrivi l'intestazione se necessario
        output_file.write("# x1_1 x1_2 x1_3 x2_1 x2_2 x2_3 x3_1 x3_2 x3_3 x3_4\n")

        # Scrivi la riga nel file
        row_str = ' '.join(map(str, sample_ord[:size_best].flatten()))
    ...
    for z in range(iterations):
        ...
        with open(str(function) + '_my_' + str(instance) + '_'  + str(dimension)+ '_' + str(initial_size) + '_' + str(size_best) + '_' + str(lb) + '_' + str(ub)+ '_' + str(iterations)+ '.txt', 'a') as file:
            # Scrivi i valori separati da uno spazio sulla stessa riga
        ...
        with open(str(inp1)+ '_my_best_batch.txt', 'a') as output_file:
            # Scrivi la riga nel file
            row_str = ' '.join(map(str, sample_ord[:size_best].flatten()))
            output_file.write(row_str + '\n')
